...
class Install extends Console\AbstractCommand implements Console\Interfaces\AuthenticatedCommandInterface, BroadcastAndListen\Interfaces\AcceptsListenersInterface
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this
            ->description('install specified field provided by the Classic Fields extension')
            ->version('1.0.0')
            ->example(
                'symphony -t 4141e465 classicfields install input'
            )
        ...
        $this
            ->addInputToCollection(
                Input\InputTypeFactory::build('Argument')
                    ...
                    ->description('name of field to install.')
                    ->validator(
    ...
    {
        return 'Usage: symphony [OPTIONS]... classicfields install <field>';
    }
    ...
    public function execute(Input\Interfaces\InputHandlerInterface $input): bool
    {
        try {
            ...
            $this->broadcast(
                ...
                E_NOTICE,
                (new Message())
                    ->message(sprintf('Installing field %s ... ', $field->name()))
