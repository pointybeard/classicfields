 ...
 */
if(false == class_exists('FieldTagList')) {
class FieldTagList extends Field implements ExportableField, ImportableField
{
    ...
    {
        if ('pre_populate_source' == $field && !is_array($value)) {
    ...
    public function getToggleStates()
    {
        ...
        foreach ($this->get('pre_populate_source') as $item) {
            ...
            $result = Symphony::Database()->fetchCol('value', sprintf(
                ...
                ('existing' == $item ? $this->get('id') : $item)
            ));
    ...
    public function commit()
    {
        ...
        $fields['pre_populate_source'] = (null === $this->get('pre_populate_source') ? 'none' : implode(',', $this->get('pre_populate_source')));
        ...
        if (is_array($this->get('pre_populate_source'))) {
            foreach ($this->get('pre_populate_source') as $field_id) {
                if ('none' === $field_id || 'existing' === $field_id) {
                    continue;
                }
    ...
    public function displayPublishPanel(XMLElement &$wrapper, $data = null, $flagWithError = null, $fieldnamePrefix = null, $fieldnamePostfix = null, $entry_id = null)
    {
        ...
        $label->appendChild(
            Widget::Input('fields'.$fieldnamePrefix.'['.$this->get('element_name').']'.$fieldnamePostfix, (0 != strlen($value) ? General::sanitize($value) : null))
        );
        ...
        if (null != $this->get('pre_populate_source')) {
    ...
    {
        if (!is_array($data) || empty($data) || null === $data['value']) {
            return;
        }
